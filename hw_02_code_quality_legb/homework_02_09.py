...
def memoize(func):
    """
    Функція декоратор для кешування результатів функцій.
    
    Параметри:
    func (function): Функція, яку необхідно кешувати.
    
    Повертає:
    function: Обгорнуту функцію з кешуванням.
    """
    global cache
...
def factorial(n, name=__name__, is_main_call=True):
    """
    Функція для обчислення факторіала з кешуванням.

    Параметри:
    n (int): Число, для якого обчислюється факторіал.
...
def fibonacci(n, name=__name__, is_main_call=True):
    """
    Функція для обчислення числа Фібоначчі з кешуванням.
    
    Параметри:
    n (int): Позиція в ряді Фібоначчі.
    
    Повертає:
    int: Число Фібоначчі на позиції n.
    """
    if n == 0:
        return 0
    elif n == 1:
